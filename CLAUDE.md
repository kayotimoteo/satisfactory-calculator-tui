...
- `src/i18n/` — the home-grown i18n layer (Intlayer was deliberately **not** used: it targets web bundlers and would jeopardize the `bun build --compile` exe, with none of its web features applying to a TUI). `messages.ts` holds one nested message table per locale (`pt-BR`, `en-US`); `Messages` is derived from `ptBR` via a `Widen<>` type so every other locale is compile-checked to have the same keys/signatures. `detect.ts` reads the OS locale (`SFCALC_LANG` override → POSIX env vars → `Intl.DateTimeFormat().resolvedOptions().locale`, the reliable signal on Windows) and maps anything starting with `pt` to `pt-BR`, else `en-US`. `I18nContext.tsx` provides `I18nProvider` + `useT()` (the message table) / `useI18n()` (locale + `setLocale`). See "Internationalization" below.
...
- **Locale is detected once at startup** from the OS and never changes during a run. `setLocale` exists on the context for a future in-app toggle but nothing calls it yet. Force a language for testing with `SFCALC_LANG=en bun start` / `SFCALC_LANG=pt bun start` (no need to change the OS).
