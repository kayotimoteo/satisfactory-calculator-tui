...
- `src/lib/clipboard.ts` — `copiarClock(valor)` formats with `fmtClockClipboard` (comma decimal, no trailing zeros — must match the original game-paste format) and shells out: `clip` on Windows, `pbcopy`/`wl-copy`/`xclip` elsewhere.
...
- `src/screens/` — one screen per tool. `TaxaScreen` is shared by both `saida` and `entrada` (a `modo` prop switches labels/math).
